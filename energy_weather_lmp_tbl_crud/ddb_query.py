...
response = table.query(
  KeyConditionExpression=Key('load_area_id').eq('DUQ') & Key('occurance_date').between(1559347200, 1577833200)
)
...
while 'LastEvaluatedKey' in response:
    ...
    response = table.query(
                    KeyConditionExpression=Key('load_area_id').eq('DUQ') & Key('occurance_date').between(1559347200, 1577833200), ExclusiveStartKey=key
                    )
    items = response['Items']
    for item in items:
        new_item = {}
